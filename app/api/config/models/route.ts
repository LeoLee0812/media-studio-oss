...
import { getStoredConfig, resolveProviderConfig } from "@/lib/config";
...
export async function POST(req: Request) {
  ...
  const apiKey = inputKey || (await resolveProviderConfig(body.provider, stored)).apiKey;
  if (!apiKey) {
    return NextResponse.json({ ok: false, models: [], error: "尚未配置 API Key，请先填入再获取" });
  }
  return NextResponse.json(await fetchProviderModels(body.provider, apiKey));
}
