...
// 由设置页的 llmProvider 决定实际打到 DeepSeek 官方 API、千问 DashScope 还是 Kimi（Moonshot）。
//
...
function buildModel(provider: LlmProvider, apiKey: string, model: string): LanguageModel {
...
export async function getLlmModel(opts: { structured?: boolean } = {}): Promise<LanguageModel> {
  const { provider, apiKey, model } = await resolveLlmConfig();
  return buildModel(provider, apiKey, effectiveModel(provider, model, opts.structured));
}

// 素材翻译模型：与文案引擎独立配置（默认云雾 deepseek-v4-flash）。
...
export async function getTranslateModel(): Promise<LanguageModel | null> {
  ...
  return buildModel(t.provider, t.apiKey, t.model);
}

// 轻量快速模型：标题重写 / 小红书高亮 / 段落 emoji 这类小任务用（延迟低、成本低）。
// 2026-07-18 起不再写死 DeepSeek 官方——走设置页「轻量任务引擎」独立配置（默认云雾
...
export async function getFlashModel(opts: { structured?: boolean } = {}): Promise<LanguageModel> {
  ...
  if (f.apiKey) return buildModel(f.provider, f.apiKey, f.model);
